...
class AdvancedTradingAnalyzer:
    ...
    def _deep_account_analysis(self, data: pd.DataFrame, sheet_name: str) -> Dict:
        ...
        # Análisis financiero avanzado - MEJORADO para coincidencia exacta con versión local
        amount_cols = [col for col in data.columns if any(word in col.lower() for word in 
                      ['amount', 'pnl', 'balance', 'profit', 'loss', 'realized', 'unrealized', 'total', 'net'])]
        
        # Detectar columna principal de PnL (prioridad específica)
        pnl_priority_keywords = ['realized_pnl', 'realized pnl', 'pnl', 'profit_loss', 'net_profit', 'amount']
        main_pnl_col = None
        
        for keyword in pnl_priority_keywords:
        ...
        # Si no se encontró columna específica, usar la primera columna numérica que contenga valores monetarios
        if not main_pnl_col and amount_cols:
            for col in amount_cols:
                if data[col].dtype in ['float64', 'int64']:
                    # Verificar que los valores sean realistas para PnL
        ...
        if main_pnl_col:
            ...
            if len(values) > 0:
                # Cálculos principales de PnL
                total_pnl = float(values.sum())
                analysis['pnl_total'] = total_pnl
                analysis['pnl_mean'] = float(values.mean())
                analysis['pnl_median'] = float(values.median())
                analysis['pnl_std'] = float(values.std())
                analysis['pnl_min'] = float(values.min())
                analysis['pnl_max'] = float(values.max())
                analysis['pnl_values'] = values.tolist()
                analysis['main_pnl_column'] = main_pnl_col
                
                # Información detallada de ganancias y pérdidas
                profits = values[values > 0]
                losses = values[values < 0]
                
                analysis['total_profit'] = float(profits.sum()) if len(profits) > 0 else 0
                analysis['total_loss'] = float(losses.sum()) if len(losses) > 0 else 0
                analysis['gross_profit'] = float(profits.sum()) if len(profits) > 0 else 0
                analysis['gross_loss'] = float(abs(losses.sum())) if len(losses) > 0 else 0
                
                # Contadores
                analysis['profitable_trades'] = len(profits)
                analysis['losing_trades'] = len(losses)
                analysis['neutral_trades'] = len(values[values == 0])
                
                # Promedios
                analysis['avg_profit'] = float(profits.mean()) if len(profits) > 0 else 0
                analysis['avg_loss'] = float(losses.mean()) if len(losses) > 0 else 0
...
def create_advanced_metrics_section(analysis: Dict):
    ...
    for account, data in financial_summary.items():
        if 'pnl_total' in data:
            total_pnl_real += data['pnl_total']
        if 'total_profit' in data:
            total_profit_real += data['total_profit']
        if 'total_loss' in data:
            total_loss_real += abs(data['total_loss'])  # Valor absoluto para pérdidas
        if 'total_trades' in data:
            total_trades_real += data['total_trades']
        if 'win_rate' in data:
            all_win_rates.append(data['win_rate'])
    
    # Métricas principales con valores absolutos
    col1, col2, col3, col4 = st.columns(4)
    
    with col1:
        pnl_color = "performance-excellent" if total_pnl_real >= 0 else "inactivity-alert"
        pnl_icon = "📈" if total_pnl_real >= 0 else "📉"
        st.markdown(f'''
        <div class="{pnl_color}">
            <h3>💰 PnL Total</h3>
            <h1>${total_pnl_real:,.2f}</h1>
            <p>{pnl_icon} {'GANANCIA' if total_pnl_real >= 0 else 'PÉRDIDA'}</p>
        </div>
        ''', unsafe_allow_html=True)
    
    with col2:
        avg_win_rate = np.mean(all_win_rates) if all_win_rates else 0
        wr_color = "performance-excellent" if avg_win_rate > 60 else "trading-insight" if avg_win_rate > 40 else "inactivity-alert"
        st.markdown(f'''
        <div class="{wr_color}">
            <h3>🎯 Win Rate Promedio</h3>
            <h1>{avg_win_rate:.1f}%</h1>
            <p>{'🏆 Excelente' if avg_win_rate > 60 else '⚖️ Balanceado' if avg_win_rate > 40 else '⚠️ Mejorable'}</p>
        </div>
        ''', unsafe_allow_html=True)
    ...
    with col3:
        ...
        <div class="trading-insight">
            ...
            <h1>{total_trades_real:,}</h1>
            <p>Trades ejecutados</p>
        </div>
        ''', unsafe_allow_html=True)
    
    with col4:
        profit_loss_ratio = (total_profit_real / total_loss_real) if total_loss_real > 0 else float('inf')
        ratio_color = "performance-excellent" if profit_loss_ratio > 1 else "inactivity-alert"
        st.markdown(f'''
        <div class="{ratio_color}">
            <h3>📊 Ratio P/L</h3>
            <h1>{profit_loss_ratio:.2f}</h1>
            <p>{'✅ Positivo' if profit_loss_ratio > 1 else '⚠️ Negativo'}</p>
        </div>
        ''', unsafe_allow_html=True)
    
    # Segunda fila con detalles de ganancias y pérdidas
    ...
    with col6:
        ...
        <div class="inactivity-alert">
            ...
            <h2>-${total_loss_real:,.2f}</h2>
            <p>Dinero perdido</p>
        </div>
        ''', unsafe_allow_html=True)
    
    with col7:
        profit_percentage = (total_profit_real / (total_profit_real + total_loss_real) * 100) if (total_profit_real + total_loss_real) > 0 else 0
        st.markdown(f'''
        <div class="trading-insight">
            <h4>📊 % Ganancias</h4>
            <h2>{profit_percentage:.1f}%</h2>
            <p>Del total movido</p>
        </div>
        ''', unsafe_allow_html=True)
    
    with col8:
        loss_percentage = (total_loss_real / (total_profit_real + total_loss_real) * 100) if (total_profit_real + total_loss_real) > 0 else 0
        st.markdown(f'''
        <div class="trading-insight">
            <h4>📊 % Pérdidas</h4>
            <h2>{loss_percentage:.1f}%</h2>
            <p>Del total movido</p>
        </div>
        ''', unsafe_allow_html=True)
...
def main():
    ...
    if 'analysis' in st.session_state:
        ...
        if st.sidebar.button("📥 Generar Reporte IA", key="btn_export"):
        ...
        if st.sidebar.button("🗑️ Limpiar y Empezar de Nuevo", type="secondary", key="btn_clear"):
